...
请根据用户提供的内容进行相应的操作。`),B=()=>{o.value=!o.value},D=e=>e.replace(/```xml([\s\S]*?)```/g,'<pre class="code-block"><code class="language-xml">$1</code></pre>').replace(/```([\s\S]*?)```/g,'<pre class="code-block"><code>$1</code></pre>').replace(/\n/g,"<br>"),X=()=>{M.value=!M.value,d.info(M.value?"隐私模式已启用":"隐私模式已禁用")},V=()=>{h.value=!h.value,d.info(h.value?"安全连接已启用":"安全连接已禁用")},G=()=>{ee(b.value),d.info(`已切换到 ${H(b.value)} 模型`)},H=e=>({local:"本地模型",ollama:"Ollama",deepseek:"DeepSeek",kimi:"Kimi"})[e]||e,I=(e,s)=>{_.value.push({role:e,content:s}),me(()=>{S.value&&(S.value.scrollTop=S.value.scrollHeight)})},U=async()=>{if(!p.value.trim()||m.value)return;const e=p.value;I("user",e),p.value="",m.value=!0;try{const s=`当前文档标题: "${i.currentTitle}"
当前文档内容: "${i.currentContent}"

用户问题: ${e}
请根据上述文档内容回答用户的问题。`;ee(b.value);const u=await be().sendMessage(s,{currentTitle:i.currentTitle,currentContent:i.currentContent,systemPrompt:L.value});if(I("assistant",u),u.includes("<akomaNtoso")&&u.includes("</akomaNtoso>")){const f=u.match(/```xml([\s\S]*?)```|(<akomaNtoso[\s\S]*<\/akomaNtoso>)/);if(f){const r=f[1]||f[2];n("content-update",{xml:r,text:$(r)})}}}catch(s){console.error("AI response error:",s),I("assistant","抱歉，处理您的请求时出现了错误。请稍后再试。"),d.error(`AI服务错误: ${s.message||"请求失败"}`)}finally{m.value=!1}},$=e=>{const s=document.createElement("div");return s.innerHTML=e,s.textContent||s.innerText||""},q=()=>{const e=[..._.value].reverse().find(s=>s.role==="assistant"&&s.content.includes("<akomaNtoso")&&s.content.includes("</akomaNtoso>"));if(e){const s=e.content.match(/```xml([\s\S]*?)```|(<akomaNtoso[\s\S]*<\/akomaNtoso>)/);if(s){const c=s[1]||s[2];n("content-update",{xml:c,text:$(c)}),d.success("XML已插入到编辑器")}else d.warning("未找到有效的XML内容")}else d.warning("聊天记录中未找到XML内容")},a=()=>{const e=[..._.value].reverse().find(s=>s.role==="assistant"&&s.content.includes("<akomaNtoso")&&s.content.includes("</akomaNtoso>"));if(e){const s=e.content.match(/```xml([\s\S]*?)```|(<akomaNtoso[\s\S]*<\/akomaNtoso>)/);if(s){const c=s[1]||s[2],u=new Blob([c],{type:"application/xml"}),f=URL.createObjectURL(u),r=document.createElement("a");r.href=f,r.download=`document-${new Date().toISOString().split("T")[0]}.xml`,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(f),d.success("XML文件已导出")}else d.warning("未找到有效的XML内容")}else d.warning("聊天记录中未找到XML内容")};return ae(()=>{I("assistant","您好！我是您的AI助手，专门帮助您处理AKN4UN XML格式的文档。您可以向我提问如何转换内容、优化结构或检查格式等问题。")}),ie(()=>{}),(e,s)=>(y(),k("div",we,[t("div",Ae,[s[3]||(s[3]=t("h3",null,"AI 助手",-1)),t("div",Ne,[O(t("select",{"onUpdate:modelValue":s[0]||(s[0]=c=>b.value=c),class:"model-selector",onChange:G},[...s[2]||(s[2]=[t("option",{value:"local"},"本地模型",-1),t("option",{value:"ollama"},"Ollama",-1),t("option",{value:"deepseek"},"DeepSeek",-1),t("option",{value:"kimi"},"Kimi",-1)])],544),[[ce,b.value]]),t("button",{class:"toggle-btn",onClick:B},C(o.value?"隐藏":"显示"),1)])]),O(t("div",xe,[t("div",{class:"chat-messages",ref_key:"messagesContainer",ref:S},[(y(!0),k(E,null,J(_.value,(c,u)=>(y(),k("div",{key:u,class:j(["message",c.role])},[t("div",{class:"message-content",innerHTML:D(c.content)},null,8,Ce)],2))),128))],512),t("div",Me,[O(t("textarea",{"onUpdate:modelValue":s[1]||(s[1]=c=>p.value=c),onKeydown:de(F(U,["ctrl"]),["enter"]),placeholder:"向AI助手提问，例如：帮我将这段内容转换为AKN4UN XML格式...",class:"chat-input"},null,40,Se),[[se,p.value]]),t("button",{onClick:U,disabled:m.value||!p.value.trim(),class:"send-btn"},C(m.value?"发送中...":"发送"),9,Ie)]),t("div",Fe,[t("div",Te,[t("span",{class:j(["secure-badge",{active:h.value}]),onClick:V,title:"点击切换安全连接状态"}," 🔐 安全连接 ",2),t("span",{class:j(["privacy-badge",{active:M.value}]),onClick:X,title:"点击切换隐私模式"}," 👁️ 隐私模式 ",2)]),t("div",{class:"action-buttons"},[t("button",{class:"action-btn",onClick:q,title:"将XML插入编辑器"}," 📄 插入XML "),t("button",{class:"action-btn",onClick:a,title:"导出XML文件"}," 💾 导出 ")])])],512),[[ue,o.value]])]))}},te=oe(Le,[["__scopeId","data-v-28655510"]]),Be={class:"cnblogs-fullscreen"},De={class:"inner-wrapper"},Xe={class:"main-body"},Ue={class:"content-inner"},$e={class:"content-left"},Ke={class:"article-editor-container"},Pe={class:"editor-header"},Oe={class:"editor-actions"},Ee={class:"editor-meta"},ze={class:"editor-content"},Ve={key:0,class:"xml-editor-section"},Ge={class:"xml-controls"},He={key:1,class:"visual-editor-section"},qe={class:"editor-controls"},je={class:"visual-content"},Je={key:0,class:"editor-preview"},We=["innerHTML"],Qe={class:"sidebar-right"},Ye={class:"sidebar-section"},Ze={class:"author-info"},et={class:"author-avatar"},tt=["src"],ot={class:"author-details"},at={class:"author-name"},st={class:"author-bio"},nt={class:"author-stats"},rt={class:"stat"},lt={class:"stat-number"},it={class:"stat"},ct={class:"stat-number"},ut={class:"stat"},dt={class:"stat-number"},mt={class:"sidebar-section"},pt={class:"quick-actions"},ht=Object.assign({components:{AIChatAssistant:te}},{__name:"ArticleEditor",setup(v){const l=ye(),i=ke(),n=pe(),o=R({id:"",title:"",content:"",xmlContent:"",category:"",tags:[],author:"",publishDate:"",likes:0,views:0,commentsCount:0}),p=R([]),_=R(["Vue","JavaScript","前端","后端","Node.js","React","TypeScript"]),m=R(!1),h=R({username:"",avatar:"",bio:"暂无简介",postsCount:0,articlesCount:0,commentsCount:0}),M=async()=>{try{p.value=await fe.getAllCategories()}catch(a){console.error("获取分类失败:",a),d.error("获取分类失败")}},S=async()=>{const a=l.params.articleId,e=n.user?.id||"1",s=`draft_${e}_cache`,c=a&&a!=="new"?`draft_${e}_${a}`:null;console.log("--- 开始加载文章 ---"),console.log("路由 ID:",a),console.log("用户 ID:",e);let u="server";const f=localStorage.getItem(s);if(f)try{const r=JSON.parse(f);(!a||a==="new"||a==="cache")&&(console.log("✅ 命中通用草稿缓存"),o.value={...r},o.value.xmlContent||(o.value.xmlContent=b(o.value.content)),u="local_draft")}catch(r){console.error("解析草稿缓存失败",r),localStorage.removeItem(s)}if(u==="server"&&a&&a!=="new"&&a!=="cache"){if(c){const r=localStorage.getItem(c);if(r){console.log("✅ 命中特定文章缓存");const A=JSON.parse(r),re=n.user?.username||n.user?.name||"墨语";A.author===re?(o.value=A,u="local_specific"):console.log("❌ 缓存文章不属于当前用户，忽略该缓存")}}if(u==="server")try{console.log("🌐 请求服务器数据...");const r=n.user?.username||n.user?.name||"墨语",A=await P.getPostByIdAndAuthor(a,r);o.value={...A},o.value.xmlContent||(o.value.xmlContent=b(A.content||A.title))}catch(r){console.error("加载文章失败:",r),d.error("加载文章失败，可能文章不存在")}}u==="server"&&(!a||a==="new")&&(console.log("📝 初始化全新文章"),o.value={id:"",title:"",content:"",xmlContent:b(),category:"",tags:[],author:n.user?.username||n.user?.name||"墨语",publishDate:new Date().toISOString().split("T")[0],likes:0,views:0,commentsCount:0}),console.log("最终加载到的数据:",o.value),console.log("数据来源:",u)},b=(a="")=>{const e=new Date().toISOString().split("T")[0];return`<?xml version="1.0" encoding="UTF-8"?>
<akomaNtoso xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
  <document type="bill">
    <meta>
      <identification source="#source">
...
</akomaNtoso>`},L=()=>{m.value=!0},B=()=>{m.value=!1},D=()=>{try{if(new DOMParser().parseFromString(o.value.xmlContent,"text/xml").getElementsByTagName("parsererror").length>0)throw new Error("XML格式错误");d.success("XML格式正确")}catch(a){d.error(`XML格式错误: ${a.message}`)}},X=()=>{try{const e=new DOMParser().parseFromString(o.value.xmlContent,"text/xml");let u=new XMLSerializer().serializeToString(e).replace(/></g,`>
<`).replace(/^(\s*)/gm,function(f,r){const A=f.length/2-1;return"  ".repeat(Math.max(0,A))});o.value.xmlContent=u,d.success("XML已格式化")}catch(a){d.error(`格式化失败: ${a.message}`)}},V=async()=>{try{const a=n.user?.id||"1";let e;!o.value.id||o.value.id==="new"||o.value.id==="cache"?e=`draft_${a}_cache`:e=`draft_${a}_${o.value.id}`;const s=n.user?.username||n.user?.name||"墨语",c={...o.value,author:s,updatedAt:new Date().toISOString()};localStorage.setItem(e,JSON.stringify(c)),console.log("✅ 文章已保存至本地缓存"),console.log("缓存键:",e),d.success("文章已保存到本地草稿箱")}catch(a){console.error("保存文章失败:",a),d.error("保存文章失败")}},G=()=>{const e=`draft_${n.user?.id||"1"}_cache`;Q.confirm("确定要清空当前编辑的所有内容吗？此操作不可恢复。","警告",{confirmButtonText:"确定",cancelButtonText:"取消",type:"warning"}).then(async()=>{o.value={id:"",title:"",content:"",xmlContent:b(),category:"",tags:[],author:n.user?.username||n.user?.name||"墨语",publishDate:new Date().toISOString().split("T")[0],likes:0,views:0,commentsCount:0},localStorage.removeItem(e),d.success("内容已清空")}).catch(()=>{})},H=async()=>{try{const a=await P.getAllPosts(),e=n.user?.username||n.user?.name||"墨语",s=a.filter(u=>u.author===e).map(u=>parseInt(u.id)).filter(u=>!isNaN(u));return(s.length>0?Math.max(...s):0)+1}catch(a){return console.error("获取下一个ID失败:",a),1}},I=async()=>{try{const a=n.user?.username||n.user?.name||"墨语",e={...o.value,author:a,xmlContent:o.value.xmlContent,publishDate:o.value.publishDate||new Date().toISOString().split("T")[0],updatedAt:new Date().toISOString()};if(!o.value.id||o.value.id===""){const s=await H();e.id=s.toString();const c=await P.createPost(e);o.value={...c},d.success("文章已发布")}else await P.updatePost(o.value.id,e),d.success("文章已发布")}catch(a){console.error("发布文章失败:",a),d.error("发布文章失败")}},U=async()=>{const a=n.user?.id||"1";if(console.log("准备返回文章列表"),console.log("当前文章ID:",o.value.id),console.log("用户ID:",a),!o.value.id||o.value.id===""||o.value.id==="cache"){const e=`draft_${a}_cache`;if(console.log("检查临时草稿键:",e),console.log("临时草稿是否存在:",localStorage.getItem(e)),localStorage.getItem(e))try{await Q.confirm("检测到有未发布的草稿，是否在离开前清除？","提示",{confirmButtonText:"清除",cancelButtonText:"保留",type:"info"}),localStorage.removeItem(e),console.log("已清除临时草稿")}catch{console.log("用户选择保留草稿")}}i.push(`/dashboard/${a}`)},$=async()=>{try{if(n.user)h.value={...n.user};else{const a=await Re.getUserById(0);h.value=a}}catch(a){console.error("获取用户信息失败:",a)}},q=a=>{a.xml&&(o.value.xmlContent=a.xml),a.text&&(o.value.content=a.text),d.success("内容已通过AI助手更新")};return ae(async()=>{await M(),await S(),await $()}),(a,e)=>{const s=K("el-input"),c=K("el-button"),u=K("el-option"),f=K("el-select");return y(),k(E,null,[t("div",Be,[t("div",De,[g(he),t("div",Xe,[t("div",Ue,[t("div",$e,[t("div",Ke,[t("div",Pe,[g(s,{modelValue:o.value.title,"onUpdate:modelValue":e[0]||(e[0]=r=>o.value.title=r),placeholder:"请输入文章标题",class:"title-input"},null,8,["modelValue"]),t("div",Oe,[g(c,{onClick:U},{default:w(()=>[...e[5]||(e[5]=[N("返回",-1)])]),_:1}),g(c,{type:"primary",onClick:V},{default:w(()=>[...e[6]||(e[6]=[N("保存",-1)])]),_:1}),g(c,{type:"danger",onClick:G},{default:w(()=>[...e[7]||(e[7]=[N("清空内容",-1)])]),_:1}),g(c,{type:"success",onClick:I},{default:w(()=>[...e[8]||(e[8]=[N("发布",-1)])]),_:1})])]),t("div",Ee,[g(f,{modelValue:o.value.category,"onUpdate:modelValue":e[1]||(e[1]=r=>o.value.category=r),placeholder:"选择分类",class:"category-select"},{default:w(()=>[(y(!0),k(E,null,J(p.value,r=>(y(),W(u,{key:r.id,label:r.name,value:r.name},null,8,["label","value"]))),128))]),_:1},8,["modelValue"]),g(f,{modelValue:o.value.tags,"onUpdate:modelValue":e[2]||(e[2]=r=>o.value.tags=r),multiple:"",placeholder:"选择标签",class:"tags-select"},{default:w(()=>[(y(!0),k(E,null,J(_.value,r=>(y(),W(u,{key:r,label:r,value:r},null,8,["label","value"]))),128))]),_:1},8,["modelValue"])]),t("div",ze,[m.value?(y(),k("div",Ve,[e[12]||(e[12]=t("h3",null,"AKN4UN XML 编辑器",-1)),t("div",Ge,[g(c,{onClick:B,size:"small"},{default:w(()=>[...e[9]||(e[9]=[N("切换到可视化编辑",-1)])]),_:1}),g(c,{onClick:D,size:"small"},{default:w(()=>[...e[10]||(e[10]=[N("验证XML",-1)])]),_:1}),g(c,{onClick:X,size:"small"},{default:w(()=>[...e[11]||(e[11]=[N("格式化",-1)])]),_:1})]),O(t("textarea",{"onUpdate:modelValue":e[3]||(e[3]=r=>o.value.xmlContent=r),class:"xml-textarea",placeholder:"在此输入AKN4UN XML内容..."},null,512),[[se,o.value.xmlContent]])])):(y(),k("div",He,[e[14]||(e[14]=t("h3",null,"可视化编辑器",-1)),t("div",qe,[g(c,{onClick:L,size:"small"},{default:w(()=>[...e[13]||(e[13]=[N("切换到XML编辑",-1)])]),_:1})]),t("div",je,[g(s,{modelValue:o.value.content,"onUpdate:modelValue":e[4]||(e[4]=r=>o.value.content=r),type:"textarea",rows:15,placeholder:"在此输入文章内容..."},null,8,["modelValue"])])]))]),m.value?ge("",!0):(y(),k("div",Je,[e[15]||(e[15]=t("h3",null,"预览",-1)),t("div",{class:"preview-content",innerHTML:o.value.content},null,8,We)]))])]),t("div",Qe,[t("div",Ye,[e[19]||(e[19]=t("h3",{class:"sidebar-title"},"博主信息",-1)),t("div",Ze,[t("div",et,[t("img",{src:h.value.avatar,alt:"博主头像",class:"avatar-img"},null,8,tt)]),t("div",ot,[t("h4",at,C(h.value.username),1),t("p",st,C(h.value.bio),1),t("div",nt,[t("div",rt,[t("span",lt,C(h.value.postsCount),1),e[16]||(e[16]=t("span",{class:"stat-label"},"随笔",-1))]),t("div",it,[t("span",ct,C(h.value.articlesCount||0),1),e[17]||(e[17]=t("span",{class:"stat-label"},"文章",-1))]),t("div",ut,[t("span",dt,C(h.value.commentsCount||0),1),e[18]||(e[18]=t("span",{class:"stat-label"},"评论",-1))])])])])]),e[21]||(e[21]=t("div",{class:"sidebar-section"},[t("h3",{class:"sidebar-title"},"编辑器提示"),t("ul",{class:"tips-list"},[t("li",null,"支持AKN4UN XML格式编辑"),t("li",null,'点击"验证XML"确保格式正确'),t("li",null,'"格式化"功能可美化XML代码'),t("li",null,"记得定期保存您的作品")])],-1)),t("div",mt,[e[20]||(e[20]=t("h3",{class:"sidebar-title"},"快捷操作",-1)),t("ul",pt,[t("li",null,[t("a",{href:"#",onClick:F(D,["prevent"])},"验证XML")]),t("li",null,[t("a",{href:"#",onClick:F(X,["prevent"])},"格式化XML")]),t("li",null,[m.value?(y(),k("a",{key:1,href:"#",onClick:F(B,["prevent"])},"切换到可视化模式")):(y(),k("a",{key:0,href:"#",onClick:F(L,["prevent"])},"切换到XML模式"))])])])])])]),g(ve)])]),g(te,{"current-content":o.value.content,"current-title":o.value.title,onContentUpdate:q},null,8,["current-content","current-title"])],64)}}}),vt=oe(ht,[["__scopeId","data-v-cfb713a7"]]);export{vt as default};
//# sourceMappingURL=ArticleEditor-CHb_xweC.js.map
